...
def verif_balise_html(html: str) -> bool:
    ...
    """
    html = ignore_balises_non_voulues(
        html)  # On ignore les balises non voulues
    ...
    for i in range(len(html)):  # On parcourt le code HTML
        if html[i] == "<" and html[i+1] != "/":  # Si on trouve une balise ouvrante
            ...
            if sommet != nom_balise_fermante:  # On vérifie si le nom de la balise fermante correspond au nom de la balise ouvrante
