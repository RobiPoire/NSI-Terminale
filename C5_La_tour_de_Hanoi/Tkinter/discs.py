...
class Disc:
    ...
    def drawDisc(self):
        ...
        centerConstant = (self.canva.winfo_width() - (self.sizeDiscWidth * len(self.Discs) + spaceBetweenTowers) * 3) / 2
        centerDiscConstant = (self.sizeDiscWidth * (self.discNumber - self.size)) / 2
        self.X = (self.sizeDiscWidth + spaceBetweenTowers) * len(self.Discs) * self.rowX + centerConstant + centerDiscConstant
    ...
    def move(self, stemList: list, stem: str):
        ...
        for i in range(len(stemList)):
            if stemList[i] == self.name: # Si le disque est sur la tige
                self.rowY = (len(self.Discs) - len(stemList)) + i # On défini la rangée
